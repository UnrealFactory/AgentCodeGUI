  ...
  },
  {
    ...
    desc: (
      <>
        ...
        <b>변경/일반 토글(Ctrl+D)</b>로 diff를 켜고 끄며 제자리에서 고치고, TS·Python과 언리얼{' '}
        <b>Verse</b>는 물론 <b>C#은 솔루션째</b>(UnrealSharp 포함) 로드해 <b>크로스 프로젝트
        정의 이동과 100% 시맨틱 색칠</b>, 소스 없는 어셈블리 심볼도 <b>F12로 디컴파일 소스까지</b>{' '}
        들어가요 — <b>공식 문서는 전면 한국어 번역과 카드 서식</b>(코드 예시 색칠·용어 풀이)으로,
        파일을 <b>여는 즉시</b> 반응하고 분석은 디스크 캐시로 <b>다시 켜도 거의 즉시</b> 떠요.
        <b>HTML은 여는 순간 렌더링된 페이지로</b>(Ctrl+D로 코드와 전환, 에이전트가 고치면 자동
        새로고침) 뜨고요. 탐색기는 <b>bin·obj 같은 빌드 폴더에 파일·*.확장자까지 숨겨</b> 소스만 남기고(우클릭{' '}
        <b>숨김 목록에 추가</b> · 설정 Explorer), <b>가장자리를 끌어 폭까지</b> 맞출 수 있어요.
        에이전트가 만진 파일은 폴더 <b>우클릭 → 변경된 파일 보기</b>로 — <b>전체/새 파일/수정됨</b>을
        나눠 한 창에 모아 diff로 바로 열어요.
      </>
    )
  },
  {
  ...
  },
  {
    ...
    desc: (
      <>
        ...
        프롬프트까지. <b>Bash 결과는 명령을 눌러</b> 명령/출력이 나뉜 <b>큰 로그 카드</b>로 읽어요.
        실행 중에도{' '}
        ...
        <b>/ask</b>가 됩니다. Claude가 뒤에서 돌리는 것도 놓치지 않아요 — <b>서브에이전트</b>는
        카드에서 <b>실시간 내레이션과 과정 로그</b>로, <b>백그라운드 셸</b>은 전용 칩에서{' '}
        <b>라이브 출력·중지·기다리는 명령 건너뛰기(Ctrl+B)</b>까지. 채팅·코드·멀티 어디서나 똑같이.
      </>
    )
  },
  {
    ...
    desc: (
      <>
        ...
        <b>패널마다 따로</b>, <b>추가 채팅 창에도</b> 있어요. 설정 <b>Account</b>에선 <b>클로드 구독
        계정을 여러 개 등록·전환</b>(로그인/로그아웃)하고, 구독일 땐 <b>채팅마다 실행 계정</b>을 따로
        고를 수도 있어요 — 계정마다 <b>다른 색</b>과 <b>남은 한도(5시간·주간·Fable)</b>가 붙어 여유
        있는 쪽을 골라 씁니다. <b>API</b>에선 키(<b>암호화 저장</b>)·예산과 <b>모델별·일별 사용
        통계</b>를 관리하고, API 모드에선 5시간 한도 대신 <b>대화 비용과 남은 예산</b>이 보여요.
      </>
    )
  },
  {
    ...
    desc: (
      <>
        ...
        테마, 최대화 버튼의 <b>창 스냅 배치</b>, 우클릭 <b>“AgentCodeGUI로 열기”</b>, 뷰어·채팅·카드
        어디서나 통하는 <b>우클릭 드래그 마우스 제스처</b>(맨 위/아래·이전/다음 파일·추가 채팅
        열기·대화 비우기·창 최대화·닫기), 새 버전이
        오면 <b>사이드바에 배지가 떠 클릭 한 번에 조용히 설치</b>되는 자동 업데이트까지. 자, 이제{' '}
        <b>시작할 시간</b>이에요.
      </>
    )
  }
