window.PROGRESS = {
  phase: 'R12 — 배선 R2(T22 유령 잠금·session-wins 영속) / M5 R2(오염가드) / 렌더러 1단계 크리틱 (병렬 3)',
  note: 'R11 판정 — 배선: 조건부 불합격(잰 것은 전부 재현·A/B/D 통과, 그러나 카드 중/스트리밍 중 CLI 사망=영구 잠김 치명 2건 — T22가 재생 테스트에만 있고 제품 배선 0 = P8 유령 그대로. +12MB는 측정 드리프트로 반박, 실재분 ~2MB) / M5: 조건부 합격(재로그인 유발 결함 0 — 단 오염가드가 계정 순서 의존, usage 파서 12/71 불일치) / 렌더러 1단계 착지: 다이얼 1~6·visibleSlots 교체·setVisible 관문·사이드바 2섹션·busy 전환 허용(+꼬리 수집기 — 2.6.2의 언로드 스윕 경쟁까지 발견·수정)·1↔6↔1 무손실 PoC PASS·live green 유지. R12가 치명 수정+렌더러 크리틱 진행 중',
