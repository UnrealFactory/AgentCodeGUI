window.PROGRESS = {
  phase: 'R11 — 배선 라이브 공격 크리틱 + M5 크리틱 + M-UX 렌더러 1단계(다이얼 1~6·사이드바 통합·그리드=뷰) 병렬 3',
  note: '★R10 이정표 — 라이브 세로 조각 PASS(실 CLI: 부팅→컴포저 타이핑→스트리밍 표시→승인 카드 왕복(무응답 영구 정지 계약 확인)→파일 실생성→재시작 후 대화·세션 보존, 결함 0, spawns 1/exits 1). chat:* 26/32 배선+별칭 19명령, CCG_UNIFIED_STORE 기본 on(탈출구 =0). 주 게이트: 켬 431.3/247.2(Priv ✓) — 통합 경로 +12MB는 원인 분해 대기. M5: 실홈 6계정 복호 6/6·왕복 바이트 동일·비관리자 정션(FSCTL 직타)·한도 조회 경로 실측(OAuth usage API·codex app-server rateLimits). 남은 큰 것: EngineEvent 9종(워크플로·bg·서브에이전트)·부팅 큐/한도 재장전·창 자리 채널',
