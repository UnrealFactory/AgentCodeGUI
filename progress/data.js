window.PROGRESS = {
  phase: 'R10 배선 라운드 — 엔진 IPC 배선(세로 조각: 부팅→대화→승인→저장 실증)+통합 스토어 기본값 전환 / M5 계정 크레이트 (병렬 2)',
  note: 'R9 확인: 3갈래 전부 [확인] — 엔진 반증 재실행 5/5 RED(순서 공격까지 추가 통과)·뮤테이션 3건 지정 테스트 사망·스토어 하네스 6종 재현+실홈 PASS+D8은 크리틱이 실앱 프로브로 직접 확증·유리 폴백 4경로 재현·멀티 회귀 0. 기본값 전환 전제 1건(R8-1: session-wins:changed가 열린 창만 실음 — 클릭 한 번에 영속 항목 증발 실측) → R10 배선 라운드가 수정 후 전환. M5 계정 크레이트 병렬 착수(OSCrypt 승계·CONFIG_DIR 물질화·한도 조회)',
