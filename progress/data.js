window.PROGRESS = {
  phase: 'R4 수정 라운드 — M1(하네스·NetSvc2 FPS·크래시 복구) + M-UX R2 + M-LOGIC R2 (병렬 3)',
  note: '크리틱 판정 착지: M1은 레버 사냥 종료(목표 356이 WebView2 빈 문서 바닥값 351~387 아래/선상 — 목표·지표는 사용자 결정), 신규 출시 블로커 발견(렌더러 크래시 시 유령 창·복구 불가 — single-process는 권장 안 함), 공짜 레버 NetSvc2 추가 발견(FPS 판정만 남음). 설계 2건은 「보완」 — 구멍 목록대로 R2 개정 중. ChatRef vs chatId는 chatId로 확정',
  updatedAt: '2026-08-22 20:55',
  metrics: [
    { name: '★ 멀티 4패널 유휴 WS', unit: 'MB', base: 712.8, new: 475.7, target: '≤356 (옵트인 single-process면 358.8)', pass: false },
    { name: '★ 멀티 4패널 유휴 Private', unit: 'MB', base: 505.3, new: 251.6, target: '≤253 (5회 중 1회 258.4)', pass: true },
    { name: '★ 멀티 4패널 + 추가 창 2개 WS', unit: 'MB', base: 934.1, new: 521.1, target: '≤467', pass: false },
    { name: '★ 4패널 동시 스트리밍 직후 WS', unit: 'MB', base: 1161, new: null, target: '≤580' },
    { name: '★ 창 1개 추가 비용', unit: 'MB', base: 110.7, new: 22.7, target: '≪110 (+0프로세스, 2.6.2는 +2)', pass: true },
    { name: '★ 멀티 스크롤 FPS', unit: 'fps', base: 60, new: 60, target: '≥60 · 드랍 0% (6회 중 1회 0.8% — 앰비언트 추정, 게이트 강화 필요)', pass: true },
    ...
    { name: '콜드 스타트 → 첫 창 (웜 중앙값)', unit: 'ms', base: 336, new: 314, target: '≤168 — 구조적 불가(웹 런타임 기동 202~240ms가 하한, Electron도 210ms)', pass: false },
    { name: '콜드 스타트 → UI 사용 가능', unit: 'ms', base: 422, new: 397, target: '≤211 — 이론 하한 258ms', pass: false },
