window.PROGRESS = {
  phase: 'R18 착지(크리틱 3 전부 "수치 재현·행복 경로 밖 관통") → node_modules 복구 후 R19 수정 라운드(M7·M4·M8 R2 + M-UI 크리틱)',
  note: 'R18 크리틱 판정 — 세 조각 모두 "잰 수치는 전부 재현(불일치 0), 행복 경로 밖에서 관통": M7 치명 2(서버 사망 시 회복 없음+status가 ready 거짓말 — 2.6.2는 30초 복귀 / 편집 버퍼 text 폐기로 저장 전 호버·정의 오답 6분의 1) + rpc.rs 설계 게이트 위반(pyright에 필요한 workspace/configuration 응답 불가) / M4 치명 2(엔진 전환 시 세션 신원이 안 갈려 첫 턴 사망 — "runtime.rs 0줄" 명제가 만든 구멍 / error turnId 게이트 미이식=결과 카드 2장) / M8 치명 1(팝아웃 닫기가 마지막 답변을 화면·디스크에서 영구 삭제 — 가장 자연스러운 사용 동작, 한 줄 가드 가능) + X=숨김 안내 부재 격상(도달 경로 겉보기 0) / M-UI 실적용 완료(7종+블라인드 54쌍 생성, 크리틱 대기). node_modules 재증발(3번째, 동시 npm 충돌 추정) — 복구 중',
  ...
  metrics: [
    ...
    { name: '★ 멀티 4패널 유휴 Private', unit: 'MB', base: 505.3, new: 253.2, target: '≤253 선상(비 0.501) — 조용한 세션 5회 중앙값, 이벤트·큐 영속·fs/git·유리·크래시 복구 전부 실린 시점. 잔여 0.2MB는 WS 재협상과 함께 사용자 결정', pass: true },
  ...
  pieces: [
    ...
    { name: 'M4 Codex 엔진+버전 관리', scope: 'app-server JSONL·엔진 설치/업데이트 — EngineAxis::Codex(O4)로 같은 상태기계 위에', state: 'build', round: 1 },
    ...
    { name: 'M7 코드 탐색기/LSP', scope: 'TS/JS·Python·C#·C++ (Verse 제외) · 수치 판정(첫 페인트·p50/p95·프리웜·유휴 회수) · ServerSpec 1항목=언어 1개', state: 'build', round: 1, critic: 'R1: bench/lsp.mjs 하네스 → 2.6.2 기준 박제 → TS 끝까지 개통' },
    ...
    { name: 'M8 멀티 창 표면', scope: '팝아웃(뷰 이동 모델)·토스트·트레이·X=숨김 — 추가 채팅 창·session-wins 영속은 R10~R12에 선완료', state: 'build', round: 1 },
