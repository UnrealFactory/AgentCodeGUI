window.PROGRESS = {
  phase: 'R26 종반 — M11 R4(실 2.6.2 상대 CAS) / M10 R3 재공격 확인 / M6 ccg-page·첨부 저장 → 다음은 M12 패키징+최종 A/B',
  ...
  pieces: [
    ...
    { name: 'M3 Claude 엔진', scope: 'ccg-engine: RunIdentity 3타입·60전이 표·라이브 원장·워치독·claude.exe 드라이버(job object 1커밋)·재생=cargo test', state: 'build', round: 5, critic: '배선 완료(세로 조각 PASS)·T22 유령 잠금·한도 안전장치 복원(오탐 0·헛 재개 0)·엔진 174 테스트. 남음: 스트리밍 메모리 게이트(실전 측정)' },
    ...
    { name: 'M7 코드 탐색기/LSP', scope: '4언어 전부 개통(TS·Python·C#·C++) — 뷰어 5/5씩, C# 토큰 34,057 동일, ts ready 229→178·첫색칠 828→550. 확장점 정직 판정: cpp는 rpc 0·server 0(코드)·manager 34줄(새 행동 축 prepare_root=인자를 만들려면 파일이 먼저여야 하는 clangd 특성). R3의 멤버십 75줄은 재사용됨', state: 'build', round: 4, critic: '4언어 착지 — 확인 크리틱 대기' },
    ...
    { name: 'M10 신기능: 세션 간 협업', scope: 'R3 착지 — 난스를 마스킹이 아니라 제거(위조 방지를 값이 아니라 줄 구조로: 본문은 줄바꿈을 가질 수 없다는 불변식+테스트). 행동 축 2겹: 봉투 턴 권한 하한 readonly(자동승인 allowlist 우회를 계획 모드로 무력화)·회신 전용(다단 중계 차단). 크리틱 공격 8종 HELD 8/8·N7 3회 0/3(수신 모델이 '옮겨 적을 표식이 없다'고 답). 정지는 도는 턴까지 소프트 중단+못 멈춘 수를 문장이 말함. 켤 때 실측 한계 고지 카드. 인젝션은 닫히지 않았다는 사실을 문서·UI 어휘로 통일', state: 'build', round: 1 },
