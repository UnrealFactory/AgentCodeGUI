window.PROGRESS = {
  phase: 'R13 — 렌더러 R2(큐 소유권 이관) / 배선 R3(이벤트 9종·부팅 재장전) / ★M6 파일·Git·뷰어 착수 / R12 확인 크리틱 (병렬 4)',
  note: 'R12 착지 — 배선 R2: E·F green(T22 제품 배선 — CLI 외부 사망 시 0초 만에 정리·사유 안내·다음 턴 정상, 백스톱은 프로세스 생존 중 절대 미발화), session-wins 영속(재시작 생존·창 되만들기·중복 0), 다이얼로그 카드, 엔진 104·스토어 58 green / M5 R2: 5건 전부 수정(오염 양방향 검출·usage 71/71 골든·관대성), 테스트 50→67 / 렌더러 크리틱: 본체 합격(다이얼·접힘·꼬리 수집기 무결)·⑥은 치명 — App 단위 큐가 전환 순간 남의 채팅으로 발사(queue.misroute 실증) → R13 렌더러 R2가 큐를 Rust Chat.queue로 이관 중. M6(탐색기·뷰어·Git) 착수 — n1 IDE 크롬의 빈 껍데기를 채운다',
