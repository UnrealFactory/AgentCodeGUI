window.PROGRESS = {
  phase: 'R8 수정 라운드 — 엔진 게이트 실측화·뮤테이션 잠금 / 스토어 치명 3건(D1~D3)+검증 눈 교체 / 유리 폴백 상시화·기본값 반전 (병렬 3)',
  note: 'R7 크리틱 공통 판정: 구현은 견고(라이브 실증 전부 통과 — O17·job object·소프트 중단·설계 대조 184셀 일치 / 크래시·마이그레이션·유리 공격 방어)하나 "옳음을 증명하는 장치"에 맹점 — 엔진 커버리지 게이트가 선언만 셈(C1)·모델 별칭 함정 미잠금(C2), 스토어 검증이 마이그레이터와 같은 눈(D4~D5 은폐)+치명 3건(D1 P3 재발·D2 index 손상 시 대화 삭제·D3 재마이그레이션 덮어쓰기), 유리 폴백이 부팅 순간에만(reload·새 창 무방비). 알림 블라인드는 신안 8승 0패(밀도 −11.7% 정직 재계산·AA 3건만 미달). 통과 조건 = 크리틱의 공격 하네스 green',
