...
- **`app:get-initial-dir`의 경로 다듬기**(R2 D3)는 **아직 안 고쳤다.** `classify()`가
  상대 경로를 절대 경로로 올리기는 하지만 2.6.2의 `path.resolve` 상당(`.`·`..` 정규화)은
  아니다. 탐색기 컨텍스트 메뉴는 늘 절대 경로(`%V`)를 주므로 **정상 경로에서는 차이가 0**
  이고, 셸에서 손으로 친 인자에서만 갈린다. 낮음으로 남긴다.
