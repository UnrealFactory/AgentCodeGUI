...
- 3.0 주행 12회 · 2.6.2 주행 12회 — **전 주행 `auth.listAccounts().length === 0`으로 확인**.
- `usage:get`이 조회를 못 나가므로 **회전 경로가 아예 안 열린다.**
- 실 HTTP: 이 라운드의 **의도된 주행에서 0건**. 예외 하나는 §2.5의 첫 스모크 1회이며,
  그것이 바로 이 라운드에서 가장 큰 발견이 됐다(그 1회의 성격도 §2.5에 그대로 적었다).
