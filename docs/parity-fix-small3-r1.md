...
> ---
>
> **★이 문서는 세 라운드를 담는다.** §1~§5 = **R1**(`09b9bc7`) · §6~§10 = **R2**(`28eb7e0`,
> 확인 크리틱 R1이 남긴 결함 넷) · §11~§15 = **R3**(확인 크리틱 R2가 남긴 결함 셋).
>
> 앞 라운드 본문에서 **틀린 것으로 판정된 문장**은 지우지 않고 그 자리에 `★R2 정정` ·
> `★R3 정정` 인용구를 달았다(§1.2 · §2.3 · §5-1 · §6.2). 본문을 읽을 때 그 인용구를
> 같이 읽어라. 특히 수치는 라운드마다 갱신된다:
>
> | | R1(§3) | R2(§8) | **R3(§14)** |
> |---|---|---|---|
> | `cargo test -p agentcodegui` | 142/0/1 | 144/0/1 | **144/0/1** |
> | `cargo test -p ccg-fs` | — | 102/0/2 | **103/0/2** |
>
> **지금 값은 §14다.** R3가 못 수를 안 늘린 것은 못을 **더한 게 아니라 고쳤기** 때문이다
> (같은 못 셋이 이제 「무엇을」에 더해 「어느 자리에」까지 본다).
