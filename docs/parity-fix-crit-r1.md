...
→ `agentcodegui.exe` **6,431,232B · md5 `e6b562d675af3cd2b8e49ee7ceff33f6`**(아래 수치는 이 exe의 것이다).
...
새 exe는 **2,861ms**(재주행 3,128ms)에 「엔진을 설치할 수 없어요 … npm(Node.js)이 필요해요」 카드를 띄우고
...
① **`EngineGate`가 `latest`를 몰라도 물러나지 않는다.** `if (!latest) return`이 있던 자리에
   ...
   npm(Node.js)이 필요해요 — Node.js를 설치한 뒤 다시 시도하세요」, 그 아래 **채널이 값으로 준
...
| | 직전 exe | 새 exe |
|---|---|---|
| 부팅 카드 | **없음**(45초 · 표본 54개 · `.sd-title` null) | **2,861ms**(재주행 3,128ms) · 「엔진을 설치할 수 없어요」 |
...
참고로 2.6.2는 같은 판에서 4,314ms에 카드를 띄운다(크리틱 R2 §5.1). **3.0이 1.2초 빠르고,
